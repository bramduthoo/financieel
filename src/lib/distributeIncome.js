...
export async function distributeIncome({ distributions, wallets, unallocatedWalletId, sourceName, date, isAutomated = false }) {
      ...
      if (balance < cap) {
        ...
        if (excess > 0 && unallocatedWalletId) {
          ...
          transactionRows.push({
            ...
            date, note: `Cap overflow (${wallet.name}) — ${sourceName}`, is_confirmed: true,
          })
        }
        ...
        if (reducedAmount > 0) {
          ...
          transactionRows.push({
            ...
            date, note: `Income distribution (reduced) — ${sourceName}`, is_confirmed: true,
          })
        }
        if (excess > 0 && unallocatedWalletId) {
          await supabase.rpc('increment_wallet_balance', { p_wallet_id: unallocatedWalletId, p_amount: excess })
          transactionRows.push({
            wallet_id: unallocatedWalletId, type: 'credit', amount: excess,
            date, note: `Cap reduction overflow (${wallet.name}) — ${sourceName}`, is_confirmed: true,
          })
        }
      } else {
